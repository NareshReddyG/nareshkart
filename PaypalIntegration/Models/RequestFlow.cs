...
namespace PaypalIntegration.Models
{
    public class RequestFlow
    {
        ...
        public void AddNewRequest(string title, IPayPalSerializableObject requestObject = null, string description = "")
        {
            ...
            {
                Request = requestObject == null ? string.Empty : FormatJsonString(requestObject.ConvertToJson()),
                Title = title,
        ...
        public void RecordResponse(PayPal.Api.IPayPalSerializableObject responseObject)
        {
            if (responseObject != null && this.Items.Any())
            {
                this.Items.Last().Response = FormatJsonString(responseObject.ConvertToJson());
            }
        }
